...
router.post('/questions', async (req, res) => {
  try {
    ...
    if (!sampleInputs || !sampleOutputs || !Array.isArray(sampleInputs) || !Array.isArray(sampleOutputs) || sampleInputs.length !== sampleOutputs.length) {
      return res.status(400).json({ message: 'Sample inputs and outputs must be arrays of equal length' });
    }
...
router.put('/questions/:id', async (req, res) => {
  try {
    ...
    if (sampleInputs && Array.isArray(sampleInputs)) question.sampleInputs = sampleInputs;
    if (sampleOutputs && Array.isArray(sampleOutputs)) question.sampleOutputs = sampleOutputs;
...
router.post('/assignments', async (req, res) => {
  try {
    ...
    // Validate input
    if (!name || !description || !dueDate || !courseId || !totalMarks || !difficultyDistribution) {
...
router.get('/assignments/:id/preview', async (req, res) => {
  try {
    ...
    const questions = await QuestionText.find({ _id: { $in: studentAssignment.questionIds } });
...
router.post('/assignments/:id/email', async (req, res) => {
  try {
    ...
    for (const student of students) {
      try {
        // Get student's questions
        const studentAssignment = assignment.students.get(student._id.toString());
        const questions = await QuestionText.find({ _id: { $in: studentAssignment.questionIds } });
